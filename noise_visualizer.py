...
while True:
    for event in pygame.event.get():
        ...
        # Keyboard events:
            ...
            if pygame.key.get_pressed()[pygame.K_c]:
                noise_color = rand.randint(0, 255), rand.randint(0, 255), rand.randint(0, 255)
            ...
            if pygame.key.get_pressed()[pygame.K_q]:
                ...
                if scaling_bias < .2:
                    scaling_bias = .2
                refresh = True
                print("bias: " + str(scaling_bias)[0:4])
            ...
            if pygame.key.get_pressed()[pygame.K_RIGHT]:
                if generation_mode == 1:
                    new_seed = perlin.gen_seed(s_width, min_height, variability)
                    ...
                    new_seed = perlin.gen_seed_2d(s_width, min_heights, variability)
        ...
        if generation_mode == 1:
            ...
            if full_refresh: # only generate for all the chunks when user has hit R
                for chunk in chunks_2d:
                    chunk[1] = perlin.gen_seed_2d(s_width, min_heights, variability)
            else:
                chunks_2d[-1][1] = perlin.gen_seed_2d(s_width, min_heights, variability)
    ...
    if (show_debug):
        text_coords = font_object.render("X (A/D): " + str(c_x) + " | Z (W/D): " + str(c_z), False, white)
        text_octaves = font_object.render("octaves (O): " + str(octaves), False, white)
        text_base = font_object.render("base ([/]): " + str(min_height), False, white)
        text_variability = font_object.render("variability (-/=): " + str(variability), False, white)
        text_bias = font_object.render("scaling_bias (Q/E): " + str(scaling_bias)[0:4], False, white)
        text_speed = font_object.render("speed (,/.): " + str(speed), False, white)
